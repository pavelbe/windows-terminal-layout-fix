...
Ручная проверка цифровой клавиши после изменения пока ожидается.

Для снимка `Win+Shift+S` в буфере находится изображение; копирование файла
...
снова читают форматы Windows clipboard. Вставка снимка в оба TUI ещё требует
ручного подтверждения. Перезапуск WSL/Terminal и агентских сессий не выполнялся.
